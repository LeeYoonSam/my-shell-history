...
for USERNAME in "${@}"; do
	...
	if [[ "${USERID}" -lt 1000 ]]; then
		echo "Refusing to remote the ${USERNAME} account with UID ${USERID}." >&2
		exit 1
	fi
    ...
    if [[ "${ARCHIVE}" = 'true' ]]; then
		...
		if [[ ! -d "${ARCHIVE_DIR}" ]]; then
			echo "Creating ${ARCHIVE_FILE} direcotry."
		...
		if [[ -d "${HOME_DIR}" ]]; then
			...
			echo "${HOME_DIR} does not exisst or is not a directory." >&2
			exit 1
		fi
    ...
    if [[ "${DELETE_USER}" = 'true' ]]; then
		...
		if ! userdel "${REMOVE_OPTION[@]}" "${USERNAME}"; then
